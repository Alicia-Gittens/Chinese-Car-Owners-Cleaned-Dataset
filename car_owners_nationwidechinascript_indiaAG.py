...
# Configuration
columns_to_remove_in_clean = [
    'gender', 'industry', 'monthly_salary', 'marital_status', 
    'education', 'brand', 'car_series', 'car_model', 
    'configuration', 'engine_number', 'unnamed:_21', 'color'
]
chunk_size = 250000  # Adjust chunk_size to 250,000

# Function to validate email, setting any variation of "noemail" to None
def is_valid_email(email):
    """Validates the format of an email address and sets variations of 'noemail' as None."""
    if pd.isna(email):
        return False
    # Check if the email contains variations of "noemail" (e.g., "noemail", "noemailnoemail", etc.)
    if re.search(r'no\s*email', email.replace(" ", ""), re.IGNORECASE):
        return False
    pattern = r'^[\w\.-]+@[\w\.-]+\.\w+$'
...
def process_data(file_path, clean_file_prefix, garbage_file_prefix):
    ...
    for i, chunk in enumerate(pd.read_csv(file_path, chunksize=chunk_size, delimiter=',', low_memory=False)):
        ...
        try:
            ...
            # Normalize variations of "noemail" to NaN
            chunk['email'] = chunk['email'].str.lower().replace(r'no\s*email', pd.NA, regex=True)
            ...
            # Ensure the date_of_birth column is formatted as mm/dd/yyyy in the clean data
            valid_chunk.loc[:, 'date_of_birth'] = pd.to_datetime(valid_chunk['date_of_birth'], errors='coerce').dt.strftime('%m/%d/%Y')
    ...
    if all_cleaned_chunks:
        ...
        # Perform final duplication check on the entire clean dataset
        final_duplicates = final_cleaned_data.duplicated(keep=False)
        final_cleaned_data['duplicate'] = final_duplicates

        # Separate valid and duplicate rows for the final clean dataset
        clean_final = final_cleaned_data[~final_duplicates]
        duplicate_final = final_cleaned_data[final_duplicates]

        # Save final clean data without duplicates
        final_cleaned_file = f"{clean_file_prefix}_final.csv"
        clean_final.to_csv(final_cleaned_file, index=False)
        logging.info(f"Saved final cleaned dataset without duplicates to {final_cleaned_file}")

        # Save final duplicates separately if any
        if not duplicate_final.empty:
            final_duplicates_file = f"{clean_file_prefix}_duplicates.csv"
            duplicate_final.to_csv(final_duplicates_file, index=False)
            logging.info(f"Saved final duplicates to {final_duplicates_file}")
